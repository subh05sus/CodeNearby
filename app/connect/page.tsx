...
export default function ConnectPage() {
  ...
  const handleLocationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: `Failed to fetch developers: ${error instanceof Error ? error.message : 'Please try again.'}`,
        variant: "destructive",
      })
    } finally {
      setLoading(false)
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          try {
            ...
            toast({
              title: "Error",
              description: `Failed to get your location: ${error instanceof Error ? error.message : 'Please enter it manually.'}`,
              variant: "destructive",
            })
            setLoading(false)
          }
  ...
  const onSwipe = async (direction: string, developerId: string) => {
    ...
    if (direction === "right") {
      try {
        ...
        toast({
          title: "Error",
          description: `Failed to send friend request: ${error instanceof Error ? error.message : 'Please try again.'}`,
          variant: "destructive",
        })
      }
    }
    ...
    <div className="flex flex-col items-center max-w-md mx-auto min-h-[50vh]">
          ...
          >
            <div className="relative w-full h-full bg-card rounded-xl shadow-xl overflow-hidden">
              ...
              <div className="p-4">
                ...
                {developer.bio && <p className="text-sm text-muted-foreground mt-2">{developer.bio}</p>}
              </div>
            </div>
          </TinderCard>
        ))}
      ...
      <div className="flex justify-center gap-4 mt-6">
        <Button
          ...
          onClick={() => developers.length > 0 && onSwipe("left", developers[developers.length - 1].id)}
        >
        ...
        <Button
          size="lg"
          className="rounded-full p-6"
          onClick={() => developers.length > 0 && onSwipe("right", developers[developers.length - 1].id)}
        >
