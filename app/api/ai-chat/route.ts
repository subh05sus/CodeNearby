...
const RATE_LIMIT_CONFIG = {
  ...
  message:
    "You've reached the AI-Connect request limit. Please try again in 10 minutes.",
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (getRepoDetails) {
      ...
      if (cachedResponse) {
        ...
        response.headers.set(
          "X-RateLimit-Limit",
          RATE_LIMIT_CONFIG.limit.toString()
        );
        response.headers.set(
      ...
      const { aiResponse } = await generateMessage(
        `
                ...
                ${
                  repoDetails.readme && repoDetails.readme.content
                    ? `README snippet: ${repoDetails.readme.content.substring(
                        0,
                        500
                      )}...`
                    : ""
                ...
                ${
                  repoDetails.language_percentages &&
                  repoDetails.language_percentages.length > 0
                    ? `Language breakdown:
                ${repoDetails.language_percentages
                  .map((lang: any) => `${lang.language}: ${lang.percentage}%`)
                  .join("\n")}`
                    : ""
                }
    ...
    if (searchSimilarRepos) {
      ...
      const { aiResponse } = await generateMessage(
        `
                ...
                ${
                  baseRepoInfo
                    ? `The repository ${repoName} is a ${
                        baseRepoInfo.language || ""
                      } project with ${
                        baseRepoInfo.stargazers_count
                      } stars and ${baseRepoInfo.forks_count} forks.
                ${
                  baseRepoInfo.description
                    ? `It is described as: "${baseRepoInfo.description}"`
                    : ""
                }
                ${
                  baseRepoInfo.topics.length > 0
                    ? `It has the following topics: ${baseRepoInfo.topics.join(
                        ", "
                      )}`
                    : ""
                }`
                ...
                ${
                  repositories.length > 0
                    ? "Here's a summary of the repositories I found:"
                    : ""
                }
    ...
    if (fetchDetailedProfile) {
      ...
      const developerDetails = (await getGitHubUserDetails(
        username
      )) as DeveloperType | null;
      ...
      const { aiResponse } = await generateMessage(
        `
                ...
                ${
                  developerDetails.topRepositories &&
                  developerDetails.topRepositories.length > 0
                    ? `Top repositories: 
                    ${developerDetails.topRepositories
                      .map(
                        (repo: any, i: number) =>
    ...
    if (directUsernameMatch && directUsernameMatch[1]) {
      ...
      if (cachedResponse) {
        console.log(
          `Retrieved profile mention response for ${username} from cache`
        );
    ...
    if (searchPerson) {
      ...
      const { aiResponse } = await generateMessage(
        `
            ...
            ${
              developers.length > 0
                ? "Here's a summary of the profiles I found:"
                : ""
            }
    ...
    // Use different prompt paths based on whether we need to search for developers
      ...
      const { aiResponse: aiAnalysis } = await generateMessage(
        `
            You are an AI assistant helping users find developers on GitHub.
      ...
      const { aiResponse } = await generateMessage(
        `
            ...
            ${
              developers.length > 0
                ? "Here's a summary of the developers I found:"
                : ""
            }
      ...
      const cacheKey = `ai:conversation:${historyHash}:${encodeURIComponent(
        messageHash
      )}`;
