...
import { fetchGitHubUserById } from "@/lib/github";
...
function GithubCard({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    const loadGithubUser = async () => {
      try {
        const user = await fetchGitHubUserById(Number(userId));
        setGithubUser(user);
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[60vh] gap-8">
        ...
        <h2 className="text-4xl font-black uppercase tracking-tighter italic">SYNCHRONIZING_GITHUB_DATA...</h2>
      </div>
    );
  }
  ...
  if (!githubUser) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-12 text-center bg-swiss-white border-8 border-swiss-black shadow-[16px_16px_0_0_rgba(255,0,0,1)]">
        ...
        <h1 className="text-6xl font-black uppercase tracking-tighter mb-4 italic">MODULE_NOT_FOUND</h1>
        <p className="text-xl font-bold uppercase tracking-tight max-w-md opacity-60">
          THE_REQUESTED_USER_PROFILE_COULD_NOT_BE_LOCATED_IN_THE_CENTRAL_REGISTRY.
        </p>
      </div>
    );
  }
          ...
          <div className="w-full space-y-8 pt-8 border-t-8 border-swiss-black">
            <div className="bg-swiss-muted/10 p-6 border-4 border-dashed border-swiss-black/20 text-center">
              <p className="font-black text-xl uppercase tracking-tighter italic">
                STATUS: OFFLINE_EXTERNAL_ORIGIN
              </p>
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-40 mt-1">
                THIS_ENTITY_HAS_NOT_YET_INITIALIZED_CODENEARBY_PROTOCOLS
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {session && (
                <SwissButton
                  ...
                  onClick={() => {
                    ...
                    toast.success("MISSION_ACCEPTED", { description: "INVITATION_CODE_DEPLOYED_TO_CLIPBOARD" });
                  }}
                >
                  <Plus className="h-6 w-6 mr-3" /> INVITE_TO_SYSTEM
                </SwissButton>
              )}
              <Link
              ...
              >
                <SwissButton variant="secondary" className="w-full h-16 text-lg">
                  <Github className="h-6 w-6 mr-3" /> VIEW_ON_GITHUB
                </SwissButton>
              </Link>
            </div>
          </div>
